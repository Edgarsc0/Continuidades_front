...
import { Plus, UserCog, X, BadgeCheck, Loader2, Mail } from "lucide-react";
import { API } from "@/components/ua/constants";
import { toast } from "sonner";

export default function EditarTitular({ titular, open, onClose, onSaved }) {
    ...
    useEffect(() => {
        if (open && titular) {
            setSelected({ numempleado: titular.no_empleado, nombres: titular.titular_nombre });
            setQuery(titular.titular_nombre ?? "");
            setEmail(titular.email ?? "");
            setCcList(Array.isArray(titular.cc) ? titular.cc : []);
        }
    }, [open, titular]);
    ...
    const addCc = () => {
        ...
        if (!addr || ccList.includes(addr)) return;
        setCcList((prev) => [...prev, addr]);
        setCcInput("");
    };

    const removeCc = (addr) => setCcList((prev) => prev.filter((e) => e !== addr));
    ...
    const handleSave = async () => {
        ...
        setLoadingSave(true);
        try {
            const res = await fetch(`${API}/api/titulares_ua/${titular.id}/editar-titular/`, {
                method: "PATCH",
            ...
            <DialogContent showCloseButton={false} className="sm:max-w-md bg-zinc-900 border border-white/10 text-white p-0 overflow-visible gap-0">
                ...
                <div className="bg-linear-to-b from-zinc-800 to-zinc-900 px-6 pt-6 pb-5 border-b border-white/10 rounded-t-xl">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-white font-semibold text-base leading-tight">Editar Titular</h2>
                            <p className="text-white/40 text-sm mt-0.5">
                                <span className="text-[#DAA520]/80">{titular.ua_nombre}</span>
                                <span className="text-white/30"> · {titular.cd_ua}</span>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="px-6 py-5">
                    <FieldGroup>
                                ...
                                <div className="relative">
                                    ...
                                    <div className="absolute right-2.5 top-1/2 -translate-y-1/2 text-white/30">
                                        {loadingSearch
                                            ...
                                            : query && <X className="w-4 h-4 cursor-pointer hover:text-white/60" onClick={handleClearEmployee} />}
                                    </div>
                                </div>
                            ...
                            <div className="mt-1 rounded-lg border border-[#DAA520]/20 bg-[#DAA520]/5 p-3 flex items-start gap-3">
                                ...
                                <div className="min-w-0 flex-1">
                                    ...
                                    <div className="mt-1 flex flex-wrap gap-x-4 gap-y-0.5 text-xs text-zinc-400">
                                        ...
                                        {selected.personal_militar_o_civil && <span>{selected.personal_militar_o_civil}</span>}
                                        {selected.nombre_puesto_funcional && <span className="truncate">{selected.nombre_puesto_funcional}</span>}
                                    </div>
                                </div>
                                <button onClick={handleClearEmployee} className="text-zinc-500 hover:text-white transition-colors shrink-0">
                        ...
                        <Field className="mt-1">
                            <Label className="text-zinc-400 text-xs uppercase tracking-widest">CC — Con copia a</Label>
                            ...
                            {ccList.length > 0 && (
                                <div className="mt-2 rounded-lg border border-white/10 overflow-hidden">
                                    <table className="w-full text-sm">
                                        ...
                                        <tbody>
                                            {ccList.map((addr) => (
                                                <tr key={addr} className="border-b border-white/5 last:border-0 hover:bg-white/3 transition-colors">
                                                    <td className="px-3 py-2 text-zinc-300">{addr}</td>
                                                    <td className="px-2 py-2 text-right">
                                                        <button type="button" onClick={() => removeCc(addr)} className="text-zinc-600 hover:text-red-400 transition-colors">
                                                            <X className="w-3.5 h-3.5" />
                                                        </button>
                                                    </td>
                                                </tr>
                ...
                <div className="px-6 py-4 border-t border-white/10 flex justify-end gap-2 rounded-b-xl">
                    ...
                    <Button
                        type="button"
                        disabled={!selected || !email.trim() || loadingSave}
                        onClick={handleSave}
